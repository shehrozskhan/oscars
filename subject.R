...
#Read actor/actress/director info file
#Best Actor
#movie_info<-read.csv("actors_names.csv",header=TRUE)
#Best Actress
#movie_info<-read.csv("actresses_names.csv",header=TRUE)
#Best Director
movie_info<-read.csv("directors_names.csv",header=TRUE)

total_record<-length(movie_info[,1])
full.info<-NULL
for (i in 1:total_record) {
  print(paste("Film=",i))
  
  #Actor/Actress/Director name
  subject_name<-as.vector(movie_info[i,2])
  
  #Name of movie
  name<-as.vector(movie_info[i,3])
  
  #Year of movie
  year<-as.numeric(movie_info[i,1])-1
  
  #Number of nominations
  nominations<-as.vector(movie_info[i,4])
  
  #Read url
  url<-as.vector(movie_info[i,5])
...
#Best Actor
#write.csv(full.info,file="actors.csv",row.names = FALSE)
#Best Actress
write.csv(full.info,file="directors.csv",row.names = FALSE)